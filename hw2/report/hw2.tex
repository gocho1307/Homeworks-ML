...
\begin{enumerate}[leftmargin=\labelsep]
    \item \textbf{Consider $x_1$ - $x_7$ to be training observations, $x_8$ - $x_9$ to be testing observations, $y_1$ - $y_5$ to be input
          ...
          According to the \textit{FAQ}, we can use the values we calculated as a \textbf{rough proxy for posteriors}. The next step is \textbf{normalize} them so we can get the maximum a posteriori:
