...
\begin{enumerate}[leftmargin=\labelsep]
  ...
  \item \textbf{Performing a hard assignment of observations to clusters under a ML assumption, identify the silhouette of both clusters under a Manhattan distance.}
        ...
        observation $x_2$, presenting the remaining results in Table \ref{exI3-likelihood-table}.
  ...
  \item \textbf{Knowing the purity of the clustering solution is 0.75, identify the number of possible classes (ground truth).}
        ...
        If we had just one class, we have $\sum_{k=1}^2 \text{max}_j\left\{\left|C_k \cap L_j\right|\right\} = \left|C_1 \cap L_1\right| + \left|C_2 \cap L_1\right| = |C_1| + |C_2| = 4 \neq 3$, because
        every observation would be in $L_1$, and so there needs to be at least two classes.
        If we assume the minimum number of classes, which is two ($L_1$ and $L_2$), we know that the misclassified observation is in the opposing class.
        But there is also a possibility that there is a class ($L_3$), that wasn't initially considered, where the misclassified observation should be.
...
\begin{enumerate}[leftmargin=\labelsep]
  ...
  \item \textbf{Visualize side-by-side the data using: i) the ground diagnoses, and ii) the \textit{previously} learned
          $k = 3$ clustering solution. To this end, project the normalized data onto a 2-dimensional data
  ...
  \item \textbf{Considering the results from questions (1) and (3), identify two ways on how clustering can
        ...
        \vskip 0.3cm
        By analyzing our results for exercise 1, we can see that most silhouette scores were moderate, indicating that there is some evidence for clusters to be
        cohesive and well-separated. Additionally, the purity scores were high, ranging from 0.63226 to 0.67742, which signifies precise clustering in relation to the known classes.
        ...
        Given the favorable results obtained in both the quantitative evaluation (silhouette and purity scores) and the visual representations, clustering appears to be a viable method
        to analyze and categorize individuals based on their health status.
        ...
        \begin{itemize}
          \item \textbf{Identifying high-risk groups}: Clustering can group individuals based on certain risk factors for a specific disease, which can help individuals with preventive measures
                for particular diseases, such as cancer. Therefore, grouping up individuals helps to characterize the symptoms and the general population of a certain disease.

          \item \textbf{Identifying relations between diseases}: Clustering can also be used to assess how separated certain clusters are from each other. This can help to better define different diseases,
                and how they are related to each other in terms of their symptoms and other variables.
        \end{itemize}
\end{enumerate}
\end{document}
